...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    id = listOfAwesome.id;
    programmers = listOfAwesome.name;
    return (id, programmers);
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (id != null) ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ id = dev.id }}>Feature</button>
            </div>
          )
        }
      </div>
